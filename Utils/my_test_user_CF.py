...
def recommendByUserFC(file_name, userid, k=5):
    ...
    recommend_dic = {}
    for neighbor in neighbors:
        neighbor_user_id = neighbor[1]#用户id
        movies = test_dic[neighbor_user_id]
        # 相似用户看过的电影，添加到 recommend_dic
        for movie in movies:
            # print movie
            # movie[0]是电影，movie[1]是对应的评分
            if movie[0] not in recommend_dic:
                recommend_dic[movie[0]] = neighbor[0]#存储（电影id，相似度值）
            else:
                recommend_dic[movie[0]] += neighbor[0]# 如果该电影已经在里面，则相似度值累加起来，然后排序
...
if __name__ == '__main__':
    ...
    recommend_list, user_movie, items_movie, neighbors = recommendByUserFC("/Users/wuyinghao/Downloads/ml-100k/u.data",
                                                                           179, 80)
