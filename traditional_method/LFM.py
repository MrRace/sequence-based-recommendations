...
class LFM:
    ...
    def gen_negative_sample_topN(self, items: dict):
        """
        Generate negative samples，每次都选取最热门的几个物品
        :param items: Original items, positive sample
        :return: Positive and negative samples
        """
        samples = dict()
        for item, rate in items.items():
            samples[item] = 1
        posiSize = len(samples)
        n = 0
        negativeSamples = sorted(items.items(), key=itemgetter(1), reverse=True)
